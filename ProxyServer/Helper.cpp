...
RequestResult Helper::createErrorResult(std::string errorMessage, int errorCode, RSA_Communication* rsa)
{
	RequestResult requestResult{};
	ErrorResult errorResult;
	errorResult.errorMessage = errorMessage;
	errorResult.code = errorCode;
	requestResult.buffer = JsonPacketSerialization::createErrorRequestResult(errorResult, rsa);
